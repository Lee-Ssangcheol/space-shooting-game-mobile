...
function setupMobileControls() {
    ...
    canvas.addEventListener('touchstart', (e) => {
        ...
        // 플레이어 위치 업데이트 - 터치한 위치가 비행기 꼬리 끝에 오도록 조정
...
function setupTouchDragControls() {
    ...
    canvas.addEventListener('touchmove', (e) => {
        ...
        // 플레이어 위치 계산 - 터치한 위치보다 플레이어가 앞쪽에 오도록 조정 (3/4 앞쪽)
        const newX = Math.max(0, Math.min(canvas.width - player.width, touchX - player.width * (3/4)));
        const newY = Math.max(0, Math.min(canvas.height - player.height, touchY - player.height * (3/4)));
