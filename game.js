...
function setupMobileControls() {
    ...
    canvas.addEventListener('touchstart', (e) => {
        ...
        // 플레이어 위치 업데이트 - 터치한 위치보다 플레이어가 앞쪽에 오도록 조정 (2/3 앞쪽)
        player.x = Math.max(0, Math.min(canvas.width - player.width, x - player.width * (2/3)));
        player.y = Math.max(0, Math.min(canvas.height - player.height, y - player.height * (2/3)));
